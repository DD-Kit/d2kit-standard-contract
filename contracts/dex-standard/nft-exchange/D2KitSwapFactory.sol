...
import "./D2KitSwapExchange20.sol";
...
interface ID2kswapFactory20 {
  ...
  /**
   * @notice Creates a D2kSwap Exchange for given token contract
...
contract D2kswapFactory20 is ID2kswapFactory20, Ownable, IDelegatedERC1155Metadata {
  ...
  /**
   * @notice Will set the initial D2kswap admin
  ...
  function createExchange(address _token, address _currency, uint256 _lpFee) public override {
    ...
    if(_newInstance > 0) {
      require(tokensToExchange[_token][_currency][_lpFee][_newInstance - 1] == address(0x0), "NF20#1"); // D2kswapFactory20#createExchange: EXCHANGE_ALREADY_CREATED
    }

    // Create new exchange contract
    D2kswapExchange20 exchange = new D2kswapExchange20(_token, _currency, _lpFee);
