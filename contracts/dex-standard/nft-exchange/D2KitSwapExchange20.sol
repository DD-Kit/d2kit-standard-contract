...
import "./swap/contracts/interfaces/ID2kswapExchange20.sol";
 ...
 * 
 * @dev ERC-777 tokens may be vulnerable if used as currency in D2kswap. Please review the code 
 *      carefully before using it with ERC-777 tokens.
 */
contract D2kswapExchange20 is ReentrancyGuard, ERC1155MintBurn, ID2kswapExchange20, IERC1155Metadata, DelegatedOwnable {
  ...
  constructor(address _tokenAddr, address _currencyAddr, uint256 _lpFee) DelegatedOwnable(msg.sender) {
    require(
      ...
      "D2K20#1" // D2kswapExchange20#constructor:INVALID_INPUT
    );
    require(
      _lpFee >= 0 && _lpFee <= 1000,  
      "D2K20#2" // D2kswapExchange20#constructor:INVALID_LP_FEE
    );
  ...
  */
  ...
  {
    // Input validation
    require(_deadline >= block.timestamp, "D2K20#3"); // D2kswapExchange20#_currencyToToken: DEADLINE_EXCEEDED
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountBought > 0, "D2K20#4"); // D2kswapExchange20#_currencyToToken: NULL_TOKENS_BOUGHT
   ...
   * @return price Amount of currency tokens to send to D2kswap.
   */
    ...
    override public view returns (uint256 price)
  {
    ...
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "D2K20#5"); // D2kswapExchange20#getBuyPrice: EMPTY_RESERVE
    ...
    internal nonReentrant() returns (uint256[] memory currencyBought)
  {
    ...
    // Input validation
    require(_deadline >= block.timestamp, "D2K20#6"); // D2kswapExchange20#_tokenToCurrency: DEADLINE_EXCEEDED
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountSold > 0, "D2K20#7"); // D2kswapExchange20#_tokenToCurrency: NULL_TOKENS_SOLD
    ...
    require(totalCurrency >= _minCurrency, "D2K20#8"); // D2kswapExchange20#_tokenToCurrency: INSUFFICIENT_CURRENCY_AMOUNT
   ...
   * @return price Amount of currency tokens to receive from D2kswap.
   */
  ...
  {
    //Reserves must not be empty
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "D2K20#9"); // D2kswapExchange20#getSellPrice: EMPTY_RESERVE
    ...
    return numerator / denominator; //Rounding errors will favor D2kswap, so nothing to do
  }
  ...
  {
    // Requirements
    require(_deadline >= block.timestamp, "D2K20#10"); // D2kswapExchange20#_addLiquidity: DEADLINE_EXCEEDED
    ...
    for (uint256 i = 0; i < nTokens; i ++) {
      ...
      require(_maxCurrency[i] > 0, "D2K20#11"); // D2kswapExchange20#_addLiquidity: NULL_MAX_CURRENCY
      require(amount > 0, "D2K20#12"); // D2kswapExchange20#_addLiquidity: NULL_TOKENS_AMOUNT
      ...
      if (totalLiquidity > 0) {
        ...
        */
        ...
        require(_maxCurrency[i] >= currencyAmount, "D2K20#13"); // D2kswapExchange20#_addLiquidity: MAX_CURRENCY_AMOUNT_EXCEEDED
        ...
        require(maxCurrency >= 1000, "D2K20#14"); // D2kswapExchange20#_addLiquidity: INVALID_CURRENCY_AMOUNT
  ...
  {
    // Input validation
    require(_deadline > block.timestamp, "D2K20#15"); // D2kswapExchange20#_removeLiquidity: DEADLINE_EXCEEDED
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(totalLiquidity > 0, "D2K20#16"); // D2kswapExchange20#_removeLiquidity: NULL_TOTAL_LIQUIDITY
      ...
      require(currencyAmount >= _minCurrency[i], "D2K20#17"); // D2kswapExchange20#_removeLiquidity: INSUFFICIENT_CURRENCY_AMOUNT
      require(tokenAmount >= _minTokens[i], "D2K20#18"); // D2kswapExchange20#_removeLiquidity: INSUFFICIENT_TOKENS
  ...
  {
    require(_deadline >= block.timestamp, "D2K20#19"); // D2kswapExchange20#buyTokens: DEADLINE_EXCEEDED
    require(_tokenIds.length > 0, "D2K20#20"); // D2kswapExchange20#buyTokens: INVALID_CURRENCY_IDS_AMOUNT
    ...
    require(nExtraFees == _extraFeeAmounts.length, "D2K20#21"); // D2kswapExchange20#buyTokens: EXTRA_FEES_ARRAYS_ARE_NOT_SAME_LENGTH
    ...
    override public returns(bytes4)
  {
    ...
    // Users must be responsible and only use this D2kswap exchange
    ...
    if (functionSignature == SELLTOKENS_SIG) {
      ...
      require(msg.sender == address(token), "D2K20#22"); // D2kswapExchange20#onERC1155BatchReceived: INVALID_TOKENS_TRANSFERRED
      ...
      // Validate fee arrays
      require(obj.extraFeeRecipients.length == obj.extraFeeAmounts.length, "D2K20#23"); // D2kswapExchange20#buyTokens: EXTRA_FEES_ARRAYS_ARE_NOT_SAME_LENGTH
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(token), "D2K20#24"); // D2kswapExchange20#onERC1155BatchReceived: INVALID_TOKEN_TRANSFERRED
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(this), "D2K20#25"); // D2kswapExchange20#onERC1155BatchReceived: INVALID_NIFTY_TOKENS_TRANSFERRED
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(currency), "D2K20#26"); // D2kswapExchange20#onERC1155BatchReceived: INVALID_TOKENS_DEPOSITED

    } else {
      revert("D2K20#27"); // D2kswapExchange20#onERC1155BatchReceived: INVALID_METHOD
    }
    ...
    override public returns(bytes4)
  {
    ...
    require(
      ...
      "D2K20#28" // D2kswapExchange20#onERC1155Received: INVALID_ONRECEIVED_MESSAGE
    );
   ...
   */
  fallback () external {
    revert("D2K20#29"); // D2kswapExchange20:UNSUPPORTED_METHOD
  }
  ...
  function setRoyaltyInfo(uint256 _fee, address _recipient) onlyOwner public {
    ...
    require(!isERC2981, "D2K20#30"); // D2kswapExchange20#setRoyaltyInfo: TOKEN SUPPORTS ERC-2981
    require(_fee <= MAX_ROYALTY, "D2K20#31"); // D2kswapExchange20#setRoyaltyInfo: ROYALTY_FEE_IS_TOO_HIGH
    ...
    internal view returns (uint256[] memory)
  {
    ...
    if (nTokens == 1) {
      ...
      for (uint256 i = 1; i < nTokens; i++) {
        require(_tokenIds[i-1] < _tokenIds[i], "D2K20#32"); // D2kswapExchange20#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS
